package utils.pattern;

import java.util.LinkedList;
import java.util.List;

public class Zalgorithm {

    public static List<Integer> search(String text, String pattern) {
        List<Integer> matches = new LinkedList<>();
        String base = pattern + "$" + text;
        int Z[] = new int[base.length()];

        getZarr(base, Z);

        for (int i = 0; i < base.length(); ++i) {
            if (Z[i] == pattern.length()) {
                matches.add(i - pattern.length() - 1);
            }
        }

        return matches;
    }

    private static void getZarr(String fullStr, int[] Z) {
        // [L,R] make a window which matches with prefix of s
        int L = 0, R = 0;

        for (int i = 1; i < fullStr.length(); ++i) {
            // if i>R nothing matches so we will calculate. Z[i] using naive way.
            if (i > R) {
                L = R = i;
                // R-L = 0 in starting, so it will start checking from 0'th index. For example,
                // for "ababab" and i = 1, the value of R remains 0 and Z[i] becomes 0. For string
                // "aaaaaa" and i = 1, Z[i] and R become 5
                while (R < fullStr.length() && fullStr.charAt(R - L) == fullStr.charAt(R))
                    R++;
                Z[i] = R - L;
                R--;
            } else {
                // k = i-L so k corresponds to number which matches in [L,R] interval.
                int k = i - L;
                // if Z[k] is less than remaining interval then Z[i] will be equal to Z[k].
                // For example, fullStr = "ababab", i = 3, R = 5 and L = 2
                if (Z[k] < R - i + 1)
                    Z[i] = Z[k];
                    // For example fullStr = "aaaaaa" and i = 2, R is 5, L is 0
                else {
                    // else start from R and check manually
                    L = i;
                    while (R < fullStr.length() && fullStr.charAt(R - L) == fullStr.charAt(R))
                        R++;
                    Z[i] = R - L;
                    R--;
                }
            }
        }
    }
}
