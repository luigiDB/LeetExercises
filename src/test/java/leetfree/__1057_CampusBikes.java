 ...
 */
public class __1057_CampusBikes {
    @Test
    public void test1() {
        Assert.assertArrayEquals(new int[]{1, 0}, assignBikes(new int[][]{{0, 0}, {2, 1}}, new int[][]{{1, 2}, {3, 3}}));
    }

    @Test
    public void test2() {
        Assert.assertArrayEquals(new int[]{0, 2, 1}, assignBikes(new int[][]{{0, 0}, {1, 1}, {2, 0}}, new int[][]{{1, 0}, {2, 2}, {2, 1}}));
    }

    public int[] assignBikes(int[][] workers, int[][] bikes) {
        PriorityQueue<Node> pq = new PriorityQueue<>(Comparator
                .comparingInt(Node::getDist)
                .thenComparingInt(Node::getWorker)
                .thenComparingInt(Node::getBike));
        ...
        while (usedBikes.size() != bikes.length && usedWorker.size() != workers.length) {
            Node poll = pq.poll();
            if (!usedWorker.contains(poll.getWorker()) && !usedBikes.contains(poll.getBike())) {
                res[poll.getWorker()] = poll.getBike();
                usedBikes.add(poll.getBike());
                usedWorker.add(poll.getWorker());
            }
        }
        return res;
    }
    ...
    class Node {
        ...
        public Node(int worker, int bike, int dist) {
            this.worker = worker;
            this.bike = bike;
            this.dist = dist;
        }
