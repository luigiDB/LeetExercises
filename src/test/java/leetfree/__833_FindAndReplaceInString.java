...
Each replacement operation has 3 parameters: a starting index i, a source word x and a target word y.  The rule is that
 if x starts at position i in the original string S, then we will replace that occurrence of x with y.  If not, we do
 nothing.
 ...
 */
public class __833_FindAndReplaceInString {
    /**
     * Build an array of size s.length and fill with support[i] = s.charAt(i)
     * For each instruction write in the first matched cell the new substring and wipe the others matched.
     * eg.
     *                  0   1   2   3   4   5   6
     * support      =>  a   b   c   d   e   f   g
     * 0,ab,fff     =>  fff         d   e   f   g
     * 4,ef,x       =>  fff         d   x       g
     * Build the resulting string with the non empty array element
     */
}
