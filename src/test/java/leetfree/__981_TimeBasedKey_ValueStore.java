 ...
 */
public class __981_TimeBasedKey_ValueStore {
    @Test
    public void test1() {
        TimeMap kv = new TimeMap();
        kv.set("foo", "bar", 1);
        Assert.assertEquals("bar", kv.get("foo", 1));
        Assert.assertEquals("bar", kv.get("foo", 3));
        kv.set("foo", "bar2", 4);
        Assert.assertEquals("bar2", kv.get("foo", 4));
        Assert.assertEquals("bar2", kv.get("foo", 5));
    }
    ...
    private class TimeMap {
        ...
        public String get(String key, int timestamp) {
            ...
            Integer lastTimestamp = map.get(key).floorKey(timestamp);
            return (lastTimestamp == null) ? "" : map.get(key).get(lastTimestamp);
        }
    }
}
